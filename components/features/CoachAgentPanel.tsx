...
import { Bot, Loader, Send, Sparkles, X, MessageCircle } from 'lucide-react';
...
export default function CoachAgentPanel({
  ...
  return (
    <>
      {/* Floating action button — toggles chat */}
      <button
        type="button"
        onClick={() => setIsOpen((open) => !open)}
        className="fixed bottom-20 right-5 z-40 flex h-14 w-14 items-center justify-center rounded-full bg-emerald-500 text-white shadow-xl shadow-emerald-500/30 transition-transform hover:bg-emerald-600 active:scale-95 sm:bottom-6 sm:right-6"
        aria-label={isOpen ? 'Close Gymi Agent' : 'Open Gymi Agent'}
